...
class on_off_AWAC_Q_lambda_Haru_obs:
    ...
    def Q_lambda_Haru(self, env, args, reset = 'random', init_state = np.array([0,0,0,8])):
        step = 0
        self.Total_iter += 1
        self.states = []
        self.actions = []
        ...
        while step < self.num_steps_per_rollout_on: 
            ...
            while not done and step < self.num_steps_per_rollout_on: 
                action = on_off_AWAC_Q_lambda_Haru_obs.select_action(self, state)
            ...
            with torch.no_grad():
                Q1, Q2 = self.critic_target(episode_states)
    ...
    def Q_lambda_Haru_off(self, off_policy_data, ntrajs):
        states = []
        actions = []
        target_Q = []
        advantage = []
