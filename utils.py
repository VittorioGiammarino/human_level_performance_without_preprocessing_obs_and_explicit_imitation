...
# =============================================================================
    ...
    for folder in Folders:
        for experiment in range(1,11):
            Training_set_single_traj, Time_single_traj, psi_single_traj, coin_direction_single_traj, reward_single_traj, real_reward, coin_location = World.Foraging.ProcessData(folder, experiment, action_space, coins)
    ...
    return TrainingSet, Trajectories, Time, Reward, Real_Traj, Real_reward, Real_Time, Coin_location
...
# =============================================================================
    ...
    for folder in Folders:
        for experiment in range(1,11):
            Training_set_single_traj, Time_single_traj, psi_single_traj, coin_direction_single_traj, reward_single_traj, real_reward = World.Foraging_Training.ProcessData(folder, experiment, action_space, coins)
            Training_set_single_traj_together = np.concatenate((0.1*Training_set_single_traj[0:-1,:], psi_single_traj[0:-1].reshape(len(psi_single_traj[0:-1]),1), coin_direction_single_traj[0:-1].reshape(len(psi_single_traj[0:-1]),1)),1)
            TrainingSet = np.append(TrainingSet, Training_set_single_traj_together, 0)
            Time.append(Time_single_traj)
            Trajectories.append(Training_set_single_traj_together)
    ...
    return TrainingSet, Trajectories, Time, Reward, Real_Traj, Real_reward, Real_Time

def Encode_Data(TrainingSet):
    T_set = TrainingSet
    # encode psi
